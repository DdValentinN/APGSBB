...
<div class="float-left" style="padding: 10px 20px;">Re-bonjour <strong><?php if(empty($identity["first_name"]) && empty($identity["last_name"])){echo $identity["username"];}elseif(empty($identity["first_name"])){echo 'Mr. '.$identity["last_name"];}else{echo $identity["first_name"];} ?></strong>, remplissez vos fiches ci-dessous : </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('modified', 'Dernière modif') ?></th>
            ...
            <tbody>
                <?php foreach ($sheets as $sheet): ?>
                    <tr>
                        ...
                        <td class="display-flex"><?php if($sheet->sheetvalidated == 1){echo "<div class='tag success'>Validée</div>";}else{echo "<div class='tag error'>Non-validée</div>";} ?></td>
                        <td class="actions">
                            <?php if($sheet->state->id != 1 || $sheet->sheetvalidated == 1){echo $this->Html->link(__('View'), ['action' => 'clientview', $sheet->id]);}elseif($sheet->state->id == 1){echo $this->Html->link(__('Modifier'), ['action' => 'clientview', $sheet->id]);}else{echo $this->Html->link(__('Edit'), ['action' => 'clientview', $sheet->id]);}  ?>
