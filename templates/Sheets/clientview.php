    ...
    <aside class="column">
        <div class="side-nav">
            ...
            <?php if ($sheet->state->id == 1 && !$sheet->sheetvalidated): ?>
                <?= $this->Form->postLink(__('Supprimer fiche'), ['action' => 'clientdelete', $sheet->id], ['confirm' => __('Êtes-vous sûr de vouloir supprimer # {0}?', $sheet->id), 'class' => 'side-nav-item']) ?>
            <?php endif; ?>
    ...
    <div class="column-responsive column-80">
        <div class="sheets view content">
            ...
            <table>
                ...
                <tr>
                    <th><?= __('Validation') ?></th>
                    <td style="justify-content: start;" class="display-flex"><?= $sheet->sheetvalidated ? __('<div class="tag success">Validée</div>') : __('<div class="tag error">Non-validée</div>'); ?></td>
                </tr>
            </table>
            
            <div class="related">
                <h4 class="float-left"><?= __('Liste des frais') ?></h4>
            ...
            <div class="related">
                <h4 class="float-left"><?= __('Liste des frais hors-forfait') ?></h4>
                <?php if($sheet->state->id == 1): ?>
                    <?php if($sheet->sheetvalidated == false): ?>
                        <?= $this->Html->link('nouveau frais hors-forfait', ['controller' => 'Outpackages', 'action' => 'addoutpackage', $sheet->id], ['class' => 'button float-right']) ?>
                    <?php endif; ?>
                <?php endif; ?>
